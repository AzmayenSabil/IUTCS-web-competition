...
export default function PastOrderHistoryTable() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${base_url}/api/v1/client/order/past-orders/${user_id}`,
          {
        ...
        const { data } = response;
        setPastOrderHistory(data.data.clientOrderHistory);
  ...
  const data = pastOrderHistory.map((order, index) => {
  ...
  return (
    <>
      ...
      {pastOrderHistory.length > 0 && !loading ? (
        <>
          <Table
            columns={columns}
            dataSource={data}
