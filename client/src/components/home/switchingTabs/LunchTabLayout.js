import { Button, Card, Checkbox, Col, Row, Modal, Empty } from "antd";
...
import { LunchOrderContext } from "../../../utils/helper/context/OrderContext";
import PackageLoaderLayout from "../../screenLoader/PackageLoaderLayout";

export default function LunchTabLayout() {
  ...
  const [showSaveAllForDateRange, setSaveAllButtonForDateRange] =
    useState(false);
  const [currentDateLunchMenuId, setCurrentDateLunchMenuId] = useState("");
  const [currentDateLunchOrder, setCurrentDateLunchOrder] = useState(false);
  const { lunchOrder, setLunchOrder } = useContext(LunchOrderContext);

  const [totalNumberOfLunch, setTotalNumberOfLunch] = useState("");
  ...
  const handleDatePickerSubmit = async (event) => {
    ...
    if (error === "") {
      const uncheckedCards = lunchOptions
        .filter(
          (lunchData) =>
            !lunchData.isOrdered &&
            (lunchData.date === startDate ||
              lunchData.date === endDate ||
              (lunchData.date > startDate && lunchData.date < endDate))
        )
        .map((lunchData) => lunchData.menu_id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const firstPackageDate = responseData.lunchPackageList[0].date;
        const currentDate = new Date().toISOString().substring(0, 10);

        if (firstPackageDate === currentDate) {
          setCurrentDateLunchMenuId(responseData.lunchPackageList[0].menu_id);
        }

        setTotalNumberOfLunch(responseData.totalNumberOfLunch);
        const mappedOptions = responseData.lunchPackageList.map((item) => {
  ...
  const pageCount = Math.floor(totalNumberOfLunch / 18);

  useEffect(() => {}, [checkedCards, ordersOfAUser, lunchOptions]);

  const handleCheckAll = (e) => {
    setCheckAll(e.target.checked);
    if (e.target.checked) {
      const allCardIds = lunchOptions
        .filter((lunchData) => !lunchData.isOrdered)
        .map((lunchData) => lunchData.menu_id);
  ...
  const handleSaveAll = async () => {
    const checkTickets = lunchOptions
      .filter((lunchData) => checkedCards.includes(lunchData.menu_id))
      .map((lunchData) => ({
        ...lunchData,
        isSelected: true,
      }));
    ...
    setLunchOptions((prevOptions) =>
      prevOptions.map((lunchData) =>
        uniqueCheckTickets.some((item) => item.menu_id === lunchData.menu_id)
          ? { ...lunchData, isSelected: true, isOrdered: true }
          : lunchData
      )
    );
  ...
  const handleOrder = async (cardId) => {
    try {
      ...
      if (currentDateLunchMenuId === cardId) {
        setCurrentDateLunchOrder(true);
        setLunchOrder(true);
      }
      ...
      if (response.data.code === 200) {
        ...
        // Update the lunch options list with the updated order status
        const updatedLunchOptions = lunchOptions.map((lunchData) => {
          if (lunchData.menu_id === cardId) {
            return {
              ...lunchData,
              isOrdered: true,
            };
        ...
        setLunchOptions(updatedLunchOptions);
  ...
  const handleYesOrderCancelModal = async () => {
    try {
      ...
      if (currentDateLunchMenuId === selectedMenuId) {
        setCurrentDateLunchOrder(false);
        setLunchOrder(false);
      }

      if (response.data.code === 200) {
        // Update lunch options state after canceling
        setLunchOptions((prevOptions) =>
          prevOptions.map((lunchData) =>
            lunchData.menu_id === selectedMenuId
              ? { ...lunchData, isOrdered: false, isSelected: false } // Set isOrdered and isSelected to false for the canceled option
              : lunchData
          )
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setlunchEndTime(data.data[1].value);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    <>
      {/* <CurrentDateMealListLayout isLunchOrdered={currentDateLunchOrder} /> */}
      ...
      <>
        {loading ? (
          ...
          // </Spin>
          // </div>
          <PackageLoaderLayout screenLoaderText="Loading Lunch Package List..." />
        ) : dataWithinRange === 0 ? (
          <Empty
            style={{
            ...
            description={
              <p style={{ color: "#ababab" }}>
                {" "}
                There are currently no Lunch Package to display.
              </p>
            }
          />
        ) : (
                ...
                <Checkbox
                  ...
                  style={{
                    ...
                    display:
                      lunchOptions.some(
                        (lunchData) =>
                          !checkIfMenuIdExists(lunchData.menu_id) &&
                          !lunchData.isSelected
              ...
              <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                <div className="flex flex-wrap items-center">
                  <div
                    ...
                    style={{ flexWrap: "wrap" }}
                  >
                    {!lunchOptions.every((lunchData) => lunchData.isOrdered) ? (
                      <form
                      ...
                      >
                        ...
                        {!doesExist && (
                          <Checkbox
                            ...
                            style={{
                              display: lunchData.isSelected ? "none" : "flex",
                            }}
                          />
                        )}
                      </div>
